...
void JoinPolygonsWithMask(const PolygonList& oldPolys, const PolygonList& newPolys,
                          const cv::Mat_<unsigned char> motionMask,
                          const cv::Mat_<unsigned char> tilesMask, PolygonList& result)
{
...
void FixTileMask(const PolygonList& oldPolys, const cv::Mat_<unsigned char> motionMask,
...
ImageToMosaic::ImageToMosaic(const bpt::ptree& ini)
    : m_guideLines(ini),
      ...
      m_sceneToImage(ini),
#endif
      m_opencvRenderer(ini),
      m_voronoiRenderer(ini)
{
   std::string tmp;
...
void ImageToMosaic::Process(const cv::Mat_<cv::Vec3b>& input, cv::Mat_<cv::Vec3b>& output,
                            cv::Mat motionMask, const cv::Mat& motionTrans)
{
